...
https://android.googlesource.com/toolchain/binutils/+/refs/heads/donut/binutils-2.17/bfd/ecofflink.c
https://kernel.googlesource.com/pub/scm/linux/kernel/git/hjl/binutils/+/hjl/secondary/include/coff/symconst.h
https://c0de.pw/bg/binutils-gdb/blob/cdbf20f73486c66e24f322400eba877eb534ae51/gdb/mdebugread.c
'''
...
def map_relative_file_descriptor(file_data, fd, symbolic_header, rfd_num):
    ...
    offset = symbolic_header.cbRfdOffset - OFFSET + (fd.rfdBase + rfd_num)*4
    return struct.unpack('>I', file_data[offset:offset+4])[0]
...
def get_type_string(file_data, fd, symbolic_header, aux_num, name, search_for_typedef):
    ...
    if aux.ti.bt == 12: # btStruct
        ...
        if aux.ti.bt >= 36:
             print('Error unknow bt: %d' % (aux.ti.bt))
...
def get_enum_string(file_data, fd, symbolic_header, enum_sym_num):
    ...
    if start_sym.st != 28:
        print('ERROR unknown type in get_enum_string start:%d' % start_sym.st)
        return ret
    ...
    while sym_num < fd.isymBase + start_sym.index:
        ...
        if sym.st == 8: # stEnd
            ...
            print('ERROR unknown type in get_enum_string:%d' % sym.st)
            break
        sym_num += 1
    return ret
...
def get_struct_or_union_string(file_data, fd, symbolic_header, union_sym_num, search_for_typedef):
    ...
    if start_sym.st == 26: # stStruct
        ...
        print('ERROR unknown type in get_struct_or_union_string start:%d' % start_sym.st)
        return ret
    ...
    while sym_num < fd.isymBase + start_sym.index:
        ...
        if sym.st == 8: # stEnd
            ...
            print('ERROR unknown type in get_struct_or_union_string:%d' % sym.st)
            break
        sym_num += 1
    return ret
...
def print_procedure(file_data, fd, symbolic_header, proc_sym_num, is_static):
    ...
    while sym_num < fd.isymBase + fd.csym:
        ...
        if sym.st == 7: # stBlock
            ...
            if end_name == proc_name:
                break
            if end_name != '':
                # this is a stEnd for something other than the function. Let's back out and return
                sym_num -= 1
                break
            decrease_indent()
            # print('%s}' % get_indent())
